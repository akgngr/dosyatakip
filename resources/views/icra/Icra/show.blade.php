...
@section('content')
@section('header')
    <section class="content-header"><h1 style="float: left; padding-right: 15px;"><span class="text-capitalize">{{$icralar->dosya_no}}</span> </h1><h3 style="padding:0; margin:0;"><span class="text-capitalize">{{ $icralar->alacakli }} - {{ $icralar->borclu }}</span></h3></section>
@endsection
<div class="box">

    <div class="box-header hidden-print with-borox"><a type="button" class="btn btn-success pull-right" href="{{route('icra.edit', $icralar->id)}}"><i class="fa fa-edit"></i> Düzenle</a>
<button type="button" class="m-r-10 btn btn-primary pull-right" data-toggle="modal" data-target="#durum">Durum</button>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form action="{{ route('icra.update', $icralar->id) }}" method="post">
                            {{method_field('PUT')}}
                            {{csrf_field()}}
                            <input type="hidden" name="eski_durum" value="{{$icralar->durum}}">
                            <input type="hidden" name="kategori" value="{{$icralar->kategori}}">
                            <div class="box-body">
                            <div class="col-md-4">
                                ...
                                <ul class="list-group list-group-flush durum">
                                    <li class="list-group-item">
                                        Derdest
                                        <label class="switch ">
                                            <input type="radio" class="primary" name="durum" @if($icralar->durum == "Derdest") checked @endif value="Derdest">
                                    ...
                                    <li class="list-group-item">
                                        İnfaz
                                        <label class="switch ">
                                            <input type="radio" class="primary" name="durum" @if($icralar->durum == "infaz") checked @endif value="infaz">
                                            <span class="slider round"></span>
                                        </label>
                                    </li>
                                </ul>
    ...
    <div class="box-body">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading">Dosya Detayları</div>
            <div class="panel-body">
                <table class="table">
                    ...
                    <tr><th>Föy Numarası</th>
                        ...
                        @if($icralar->durum == 'Derdest')

                            @if(isset($icralar->icra_derdest_foy_no))
                                <td>{{$icralar->icra_derdest_foy_no->foy_no}}</td>
                            @else
                                <td>
                                    <form action="{{route('icra.derdest.kaydet')}}" method="post">
                                        {{csrf_field()}}
                        ...
                        @else
                            @if(isset($icralar->icra_infaz_foy_no))
                                <td>{{$icralar->icra_infaz_foy_no->foy_no}}</td>
                            @else
                                <td>
                                    <form action="{{route('icra.infaz.kaydet')}}" method="post">
                                        {{csrf_field()}}
                                        <input type="hidden" name="icra_id" value="{{$icralar->id}}">
                                        <input type="hidden" name="kategori" value="{{$icralar->kategori}}">
                                        <input type="submit" class="btn btn-success" value="Föyno Oluştur">
                        ...
                        <tr><th>Dosya No:</th><td>{{ $icralar->dosya_no }}</td></tr>
                        <tr><th>Kategorisi:</th><td>
                                @if(isset($icralar->parent))
                                @php($ust_kategori=$icralar->parent)
                                    ...
                                    @endif
                                    {{ $icralar->parent->title }}
                                @endif
                            </td></tr>
                        <tr><th>İli:</th><td>{{ $icralar->ili }}</td></tr>
                        <tr><th>Mahkeme:</th><td>{{ $icralar->mahkeme }}</td></tr>
                        <tr><th>Alacaklı:</th><td>{{ $icralar->alacakli }}</td></tr>
                        <tr><th>Borçlu:</th><td>{{ $icralar->borclu }}</td></tr>
                        <tr><th>Ekleyen Kişi</th><td>{{$icralar->user->name}}</td></tr>
                        <tr><th>Ekleme Trihi</th><td>{{$icralar->created_at}}</td></tr>
                        <tr class="@if($icralar->durum == 'infaz') danger @endif"> <th>Durum</th><td>{{$icralar->durum}}</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
<div id="not" class="tab-pane fade in active">
    ...
    @foreach($icralar->yorumlar as $yorum)
      ...
      <div class="col-md-12">
      <form action="{{route('icra.kaydet')}}" method="post">
          {{csrf_field()}}
          <input type="hidden" name='icra' value="{{$icralar->id}}">
